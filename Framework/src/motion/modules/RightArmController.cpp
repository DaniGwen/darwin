...
namespace Robot
{
    ...
    Point2D RightArmController::GetHandPositionInCameraView()
    {
        ...
        // CORRECTED: Call Value2Deg on the singleton instance
        double shoulder_pitch_rad = Robot::HeadTracking::GetInstance()->Value2Deg(shoulder_pitch_val) * (M_PI / 180.0);
        double shoulder_roll_rad = Robot::HeadTracking::GetInstance()->Value2Deg(shoulder_roll_val) * (M_PI / 180.0);
        ...
        double hand_in_cam_z = hand_x - CAM_Z_OFFSET_M;

        // --- Step 4: Project 3D Camera Point to 2D Image Plane ---
        const double FOCAL_LENGTH_PX = HeadTracking::GetInstance()->GetFocalLengthPx();
    ...
    // Replace your CenterHandInView function with this corrected version
    void RightArmController::CenterHandInView(int centering_speed)
    {
        ...
        for (int i = 0; i < MAX_ITERATIONS; ++i)
        {
            ...
            double pitch_offset_deg = -pixel_error.Y * m_Hand_P_Gain_Y;
            double roll_offset_deg = -pixel_error.X * m_Hand_P_Gain_X;
            ...
            // Convert to degrees
            double current_pitch_deg = Robot::HeadTracking::GetInstance()->Value2Deg(shoulder_pitch_val);
            double current_roll_deg = Robot::HeadTracking::GetInstance()->Value2Deg(shoulder_roll_val);

            double target_pitch_deg = current_pitch_deg + pitch_offset_deg;
            double target_roll_deg = current_roll_deg + roll_offset_deg;

            Pose next_pose;
            next_pose.joint_positions[JointData::ID_R_SHOULDER_PITCH] = Robot::HeadTracking::GetInstance()->Deg2Value(target_pitch_deg);
            next_pose.joint_positions[JointData::ID_R_SHOULDER_ROLL] = Robot::HeadTracking::GetInstance()->Deg2Value(target_roll_deg);
