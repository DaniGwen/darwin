...
LinuxCamera::LinuxCamera() : settings(CameraSettings()),
                             camera_fd(-1),
                             buffers(0),
                             n_buffers(0)
{
...
int LinuxCamera::Initialize(int deviceIndex)
{
    ...
    fps.parm.capture.timeperframe.denominator = 25; // Drop from 30 down to 15 to save USB bandwidth!
...
int LinuxCamera::ReadFrame()
{
    ...
    if (-1 == ioctl(camera_fd, VIDIOC_DQBUF, &buf))
    {
        ...
        default:
            fprintf(stderr, "Camera Driver Error: %d\n", errno);
            exit(EXIT_FAILURE);
        }
    }
