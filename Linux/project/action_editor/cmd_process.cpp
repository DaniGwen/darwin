...
void WriteStepCmd(int index)
{
	...
	bool data_written = false; // Flag to track if any valid data was written

	// Iterate through all joints (up to ID 22, as NUMBER_OF_JOINTS is 23)
	for (int id = JointData::ID_R_SHOULDER_PITCH; id < JointData::NUMBER_OF_JOINTS; id++)
	{
		// If the source (Step - STP7) has a valid position (not invalid and not torque off)
		if (!(Step.position[id] & Action::INVALID_BIT_MASK) && !(Step.position[id] & Action::TORQUE_OFF_BIT_MASK))
		{
			// Overwrite the destination step's position with the valid position from Step
			Page.step[index].position[id] = Step.position[id];
			data_written = true;
		}
		// If the source is invalid or torque off, we leave the destination step's
		// position for this joint as it is. This preserves existing valid data in the
		// page step or keeps it as invalid/torque off if it was already.
	}

	// Also handle pause and time, as these are part of the step.
	// Assuming pause and time in Step are always valid after ReadStep.
	Page.step[index].pause = Step.pause;
	Page.step[index].time = Step.time;
	data_written = true; // Pause/Time are always considered data being written

	if (data_written)
	{
		DrawStep(index); // Redraw the step to show the updated values
		bEdited = true;
	}
	else
	{
		PrintCmd("No valid joint data in STP7 to write.");
	}

	// Update stepnum if writing to a step beyond the current stepnum
	if (index >= Page.header.stepnum)
	{
		DrawStepLine(true); // Erase old line
		Page.header.stepnum = index + 1;
		DrawStepLine(false); // Draw new line
		GoToCursor(PAGEPARAM_COL, STEPNUM_ROW);
		printf("%.3d", Page.header.stepnum);
		bEdited = true;
	}
...
void InsertStepCmd(int index)
{
	...
	{
		PrintCmd("Action page is full. Cannot insert.");
		return;
	}

	// Shift steps to make space for the insert
	for (int i = Action::MAXNUM_STEP - 1; i > index; i--)
	{
		Page.step[i] = Page.step[i - 1];
		DrawStep(i); // Redraw the shifted step
	}

	// Create a new step structure to populate
	Action::STEP newStep;

	// Initialize the new step to a default state (e.g., invalid for all joints)
	for (int id = 0; id < JointData::NUMBER_OF_JOINTS; id++) // Initialize all possible joint slots
	{
		newStep.position[id] = Action::INVALID_BIT_MASK;
	}
	newStep.pause = 0; // Default pause/time
	newStep.time = 0;

	// Now, selectively copy valid data from the current live Step (STP7) into newStep
	bool data_inserted = false; // Flag to track if any valid data was inserted
	for (int id = JointData::ID_R_SHOULDER_PITCH; id < JointData::NUMBER_OF_JOINTS; id++)
	{
		// If the source (Step - STP7) has a valid position (not invalid and not torque off)
		if (!(Step.position[id] & Action::INVALID_BIT_MASK) && !(Step.position[id] & Action::TORQUE_OFF_BIT_MASK))
		{
			// Copy the valid position from Step to the new step
			newStep.position[id] = Step.position[id];
			data_inserted = true;
		}
		// If Step.position[id] is invalid or torque off, newStep.position[id] remains INVALID_BIT_MASK
		// from the initialization above.
	}
	newStep.pause = Step.pause; // Copy pause and time from Step
	newStep.time = Step.time;
	data_inserted = true; // Pause/Time are always considered data being inserted

	// Assign the created newStep to the page at the insert index
	Page.step[index] = newStep;
	DrawStep(index); // Draw the newly inserted step

	// Update stepnum if inserting within or at the end of the sequence
	if (Page.header.stepnum < Action::MAXNUM_STEP) // Only increment if not already full
	{
		DrawStepLine(true); // Erase old line
		Page.header.stepnum++;
		DrawStepLine(false); // Draw new line
	...
	{
		bEdited = true; // Mark as edited even if stepnum didn't change (content changed)
	}

	if (!data_inserted)
	{
		PrintCmd("Warning: Inserted step contains no valid joint data from STP7.");
	}
}
